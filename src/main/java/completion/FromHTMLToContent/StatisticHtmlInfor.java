 ...
 **/
public class StatisticHtmlInfor {
    private static class HtmlEntityInfor{
        String rootPath;
        String entityPath;
        int totalFileNumber;
        ...
        HtmlEntityInfor(String rootPath, String entityPath, int totalFileNumber, int lengthLegalFileNumber,
    ...
    private ArrayList<HtmlEntityInfor> fileToEntityInfor(int beginIndex, int endIndex) {
        ArrayList<HtmlEntityInfor> resultList = new ArrayList<>();

        String readExcelName = "D:/Java/Workspace/GraDesign1/src/main/resources/Keywords/htmlinfor_202107202211.xlsx";
        ...
        for (int i = beginIndex; i < endIndex; i++) {
            ...
            int lengthType = (int)readSheet.getRow(i).getCell(5).getNumericCellValue();
            int headTime = (int)readSheet.getRow(i).getCell(6).getNumericCellValue();
            int tailTime = (int)readSheet.getRow(i).getCell(7).getNumericCellValue();
            int headAndTail = (int)readSheet.getRow(i).getCell(8).getNumericCellValue();
            ...
            if(!nextEntityPath.equals(nowEntityPath)){
                HtmlEntityInfor infor = inforListToEntityInfor(rootPath,entityPath,lengthTypeList,headTimeList,tailTimeList,headAndTailList);
    ...
    private HtmlEntityInfor inforListToEntityInfor(String rootPath, String entityPath, ArrayList<Integer> lengthTypeList,
        ...
        HtmlEntityInfor entityInfor = new HtmlEntityInfor(rootPath,entityPath,totalFileNumber,lengthLegalFileNumber,
                totalHeadFileNumber,totalTailFileNumber,totalLegalFileNumber);
        return entityInfor;
    }

    //将文件的统计信息写入数据库
    private void writeInforToDB(ArrayList<HtmlEntityInfor> inforList, String sql){
        Connection conn;
        ...
        try {
            ...
            for(int i=0; i<inforList.size(); i++){
                HtmlEntityInfor infor = inforList.get(i);
    ...
    public void runStatisticHtml(int begin, int end){

        ArrayList<HtmlEntityInfor> results = fileToEntityInfor(begin, end);
        String sql = "insert into HtmlEntityInfor values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
